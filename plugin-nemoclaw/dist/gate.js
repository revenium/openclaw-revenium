...
function persistRunState(runId, marked) {
    try {
        ...
        if (!filePath)
            return; // sanitized to empty — skip silently
    ...
    if (_runStateDirOverride !== null) {
        try {
            ...
            for (const entry of entries) {
                try {
                    rmSync(join(_runStateDirOverride, entry));
                }
                catch { /* ignore */ }
            }
        }
        ...
        for (const entry of transcript) {
            try {
                ...
                for (const part of content) {
                    if (!part || part.type !== "toolCall" || part.name !== "tool_search_code")
                        continue;
...
export function handleBeforeAgentFinalize(runId, transcript) {
        ...
        if (filePath && existsSync(filePath)) {
            ...
            if (state && state.exec === true) {
                ...
                if (state.marked === true)
                    return undefined; // already classified
    ...
    if (scan.execFound) {
        if (scan.markerFound)
            return undefined; // write-marker.sh invoked in transcript
