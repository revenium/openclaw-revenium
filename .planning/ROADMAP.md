...
**Success Criteria** (what must be TRUE):
  ...
  3. *(Renegotiated 2026-06-10 per 15-VALIDATION.md round-3 evidence — see note below)* The `before_agent_finalize` marker-gate plugin (from Phase 11) is deployed/adapted for the sandbox environment and task/job markers are written for agents that invoke `exec` directly (full gateway messaging sessions). KNOWN LIMITATION: the Nemotron model on NemoClaw routes shell execution through `tool_search_code` → `openclaw.tools.call('openclaw:core:exec', ...)` and the `openclaw agent --json` CLI path uses the `agent/embedded` runner, which does not fire `before_agent_finalize`. As a result, end-to-end marker production cannot be demonstrated live via `nemoclaw exec` CLI turns on this host. The transcript-scan fix (`scanTranscriptForExec`) is correctly implemented and unit-tested (57/57), and will activate for full gateway messaging sessions (SMS, web UI, channel-mediated turns). Attribution under NemoClaw is scoped to direct-exec agents and gateway sessions — not CLI-driven `openclaw agent --json` turns.
...
- [x] 16-02-PLAN.md — docs/nemoclaw-setup.md operator runbook (D-01/D-04, verbatim macOS error) + single README pointer link (NCDEPLOY-02)
