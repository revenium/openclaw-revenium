...
**Status:** v1.4 NemoClaw/OpenShell Support — **shipped 2026-06-11**, plus **v1.4.1 install-path hardening** and a **post-ship jobs/enforcement session (2026-06-12/13)**, all on `origin/main` (tagged `v1.4`). Milestone archived 2026-06-13. NemoClaw install validated end-to-end on a clean host (exits 0). **Post-ship session** (live-validated on NemoClaw/Sonnet + vanilla/Opus): hard-halt arming via `--autonomous`/`REVENIUM_BUDGET_AUTONOMOUS`; jobs-directive injection + cron-race sweep; **declare-at-start job lifecycle** (`RUNNING`→`--close`, interval spend attribution, stale janitor); **per-turn `before_prompt_build` directive injection** as the compliance driver (OpenClaw 2026.6.6 silently vetoes `before_agent_finalize` revise on tool-using turns); one-step env-driven vanilla setup; Homebrew Linux-sandbox + cross-tenant rule-adoption guards. No active milestone; run `/gsd-new-milestone` to scope the next one.
