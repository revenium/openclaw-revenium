...
- **D-05 (revised 2026-06-04 per research finding):** **Bounded + fail-open (carry-forward).** `retry.maxAttempts` caps the forced passes (seed proposes `1`); if the agent still doesn't classify, the harness finalizes anyway. A hook error or timeout must never block the user's reply. **Privacy intent:** the plugin reads **no conversation content** — it observes `exec` tool calls only via the `before_tool_call` hook (which is *not* a conversation hook). **However**, research proved the SDK requires `plugins.entries.<id>.hooks.allowConversationAccess: true` to register `before_agent_finalize` + `agent_end` — both are `CONVERSATION_HOOK_NAMES` in OpenClaw 2026.6.1, and without the flag they are *silently blocked* (warn-level log, gate becomes a no-op). `post-install.sh` MUST set this flag, and MUST add an `openclaw plugins inspect` check that `hookNames` includes `before_agent_finalize` to catch the silent-block failure mode. The original "no `allowConversationAccess`" wording is superseded: the flag is mandatory for the chosen `before_agent_finalize` direction (D-00); only the *privacy intent* (observes exec calls, never reads conversation) carries forward.
